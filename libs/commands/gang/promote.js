exports.run = async (client, message, args) => {
	...
	if (rankPromoted == "OFFICER") return Errors.showError(message.member.displayName, message.channel, '#FF0000', 'That member already is an officer.')
	if (rankMember != "LEADER") return Errors.showError(message.member.displayName, message.channel, '#FF0000', "You can't promote other people.", message.author.avatarURL);
	...
	embed.setDescription('**Gang**\n\nAre you sure you want to promote ``' + promoted.displayName + '`` to officer?');
	  ...
	  files: [{
	    ...
	    msg.awaitReactions(filter, { max: 1, time: 60000, errors: ['time'] }).then(async (collected) => {
	      	...
	      	if (reaction.emoji.name === '👎') {
				...
				await Gangs.setRank(promoted, 2);

			    embed.setDescription('**Gang**\n\n``' + promoted.displayName + '`` has been promoted to Officer');
...
exports.config = {
	type: "gang",
	desc: client.lang.desc.gang.promote,
	usage: 'g promote <user>'
}