exports.run = async (client, message, args) => {
    ...
    embed.setAuthor(message.member.displayName, message.author.avatarURL)
   	...
   	embed.setDescription('**Gang**\n\nAre you sure you want to make the gang public?');
	  ...
	  files: [{
	    ...
	    msg.awaitReactions(filter, { max: 1, time: 60000, errors: ['time'] }).then(async (collected) => {
	      	...
	      	if (reaction.emoji.name === '👎') {
   			    ...
   			    db.query("UPDATE `gangs` SET `open`=1 WHERE `gang_name`='" + gangMember + "';", async function (err, result, fields) {
		            if (err) throw err;

		            embed.setDescription('**Gang**\n\nThe gang ``' + gangMember + '``is now OPEN and joinable for everyone.');
...
exports.config = {
	type: "gang",
	desc: client.lang.desc.gang.open,
	usage: 'g open'
}